      ...
      <>
        <div className={cx("container")}>
          {cards.map((card, index) => (
            <div className={cx("container__card")} key={card.username}>
              <img src={card.photos[0]} alt={card.username} />
              <div className={cx("container__card__data")}>
                <div className={cx("container__card__data__main")}>
                  <h1>{card.username}</h1>
                  <p data-testid="age">{card.infoAge}</p>
                </div>
                <div className={cx("container__card__data__others")}>
                  <p data-testid="gender">{card.infoGender}</p>
                </div>
                <div className={cx("container__card__data__options")}>
                  <Icon
                    size="m"
                    library="material-icons"
                    type="close"
                    onClick={() => submitDiscardSuggestion(index)}
                  ></Icon>
                  <Icon
                    size="m"
                    library="material-icons"
                    type="favorite_border"
                    onClick={() => submitBeloved(card.username, index)}
                  ></Icon>
                </div>
              </div>
            </div>
    ...
    return (
      <>
        <div className={cx("message")}>
          <h1 className={cx("message__title")}>Suggestions are coming soon!</h1>
